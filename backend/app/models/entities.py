...
class VectorType(TypeDecorator):
    """Minimal pgvector `vector(1536)` type that binds as text.

    Values are always supplied/read as the canonical string form
    `'[0.1,0.2,...]'`, so the impl is VARCHAR for both directions.
    """
...
class Exam(Base):
    ...
    conducting_body: Mapped[str | None] = mapped_column(String(150))
    exam_level: Mapped[str | None] = mapped_column(String(50))
    application_fee: Mapped[float | None] = mapped_column(Numeric(10, 2))
    embedding_vector: Mapped[str | None] = mapped_column(VectorType(dimensions=1536))
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=datetime.utcnow
    )
...
class ScrapingSource(Base):
    ...
    cron_expression: Mapped[str] = mapped_column(
        String(50), default="0 0 * * *"
    )
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)


class RawStagingPayload(Base):
    __tablename__ = "raw_staging_payloads"

    payload_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), primary_key=True, default=uuid.uuid4
    )
    source_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("scraping_sources.source_id", ondelete="CASCADE")
    )
    raw_html_or_json: Mapped[str] = mapped_column(Text, nullable=False)
...
class IngestionAnomalyLog(Base):
    __tablename__ = "ingestion_anomaly_logs"

    log_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), primary_key=True, default=uuid.uuid4
    )
    payload_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("raw_staging_payloads.payload_id", ondelete="CASCADE")
    )
